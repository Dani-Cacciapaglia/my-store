...
const requiredEnvVars = ['GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET', 'GOOGLE_REDIRECT_URL'];
const missingEnvVars = requiredEnvVars.filter(v => !process.env[v] || process.env[v].includes('your_'));
...
try {
    ...
    // Only set existing tokens if they're not placeholders
    if (process.env.GOOGLE_ACCESS_TOKEN &&
        !process.env.GOOGLE_ACCESS_TOKEN.includes('your_') &&
        process.env.GOOGLE_REFRESH_TOKEN &&
        !process.env.GOOGLE_REFRESH_TOKEN.includes('your_')) {
...
app.get('/api/availability', async (req, res) => {
    try {
        ...
        events.forEach(event => {
            // Consider as busy if event is not transparent and not cancelled
            if (event.status !== 'cancelled' && event.transparency !== 'transparent') {
                ...
                if (isAllDay) {
                    // Google returns all-day end as exclusive date; mark each day accordingly
                    start = new Date(event.start.date);
                    end = new Date(event.end.date);
                }

                // Mark all days covered by the event as unavailable
